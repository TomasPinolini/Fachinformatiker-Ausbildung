<?php
    ...
    $error_message = '';
    $registration_error = '';
    $mysqli = require __DIR__ . "/16-dbconnection.php";
            ...
            if (empty($postemail) || empty($postpassword)) {
                ...
                if ($result && $result->num_rows > 0) {
                    ...
                    if ($postpassword === $user["password"]) {
                        $_SESSION = $user;
                        if($user["type"] === "client"){
                            header("Location: 18-homepages/18-client_hp.php");
                        }elseif($user["type"] === "admin"){
                            header("Location: 18-homepages/18-admin_create.php");
                        }elseif($user["type"] === "guest"){                           
                            header("Location: 18-homepages/18-guest_hp.php");
                        }
                        exit;
                    } else {
                        $login_invalid = true;

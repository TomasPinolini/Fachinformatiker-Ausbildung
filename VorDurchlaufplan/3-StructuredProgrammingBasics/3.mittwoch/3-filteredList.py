...
def showOps():
    fil = input("""Would you like to 
1- Filter the operations by user? 
2- Not filtered?
3- Or see them in order by quantity of operations? 
->  """).capitalize()
    while fil not in ["1", "2", "3"]:
        fil = input("(1/2/3): ").capitalize()
    
    if fil == "1":
        user = input("User ID of the user you want to see the activity from: ")
        while True:
            try:
                user = int(user)
                break
            ...
            with open(opsfp, "r") as oF:
                oF.seek(0)
            print("Operations with {user} as ID:")
            for op in oF:
                info = op.split(",")
                if user == info[0]:
                    print(str(info))
